...
import { delRepairTool, listRepairTool } from '@/api/collection/repairTool.ts'
...
async function handleGetList() {
  try {
    ...
    const res = (await listRepairTool(params)) as any
...
function handleRowDelete(idList: TIdList) {
  proxy!
    .$modal!.confirm(`是否确认删除编号为${idList.join(', ')}的数据项？`)
    .then(() => delRepairTool(idList))
    .then(() => {
      handleGetList()
