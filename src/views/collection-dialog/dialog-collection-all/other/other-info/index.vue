...
import { auditOtherInfo, delOtherInfo, getOtherInfo } from '@/api/collection-all/other/other-info'
...
async function handleGetList() {
  try {
    ...
    const res = await getOtherInfo(params)
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 标题: ${msg} 数据项？`)
    .then(() => delOtherInfo({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  props.proxy.$modal
    .confirm(`是否提交审核 标题: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditOtherInfo(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
