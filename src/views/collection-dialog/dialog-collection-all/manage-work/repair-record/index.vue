<template>
  <div class="app-container">
    <!--    <div class="tip">本页仅供查看修复记录，需要增加或者修改请至“移动记录”</div>-->
    <page-header
...
import { auditRepairRecord, delRepairRecord, getRepairRecord } from '@/api/collection-all/manage-work/repair-record'
...
import { auditProtectInfo, delProtectInfo } from '@/api/collection-all/manage-work/protect-info'
...
async function handleGetList() {
  try {
    ...
    const res = await getRepairRecord(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '修复记录-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '修复记录-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleDelete() {
  ...
  const msg = selectionArr.value?.map(row => {
    return `${row?.undertakingOrganization}`
  })
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 承担机构: ${msg} 数据项？`)
    .then(() => delRepairRecord({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ...
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 承担机构: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditRepairRecord(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
