...
import { auditMoveRecord, delMoveRecord, getMoveRecord } from '@/api/collection-all/manage-work/move-record'
import CommHandleBtnTable from '@/components/comm-cpns/comm-handle-btn-table/comm-handle-btn-table.vue'
import { auditRepairRecord, delRepairRecord } from '@/api/collection-all/manage-work/repair-record'
import { auditAuthenticate } from '@/api/collection-all/manage-work/authenticate-info'
...
async function handleGetList() {
  try {
    ...
    const res = await getMoveRecord(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '移动记录-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '移动记录-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 执行单位: ${msg} 数据项？`)
    .then(() => delMoveRecord({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ...
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 执行单位: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditMoveRecord(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
