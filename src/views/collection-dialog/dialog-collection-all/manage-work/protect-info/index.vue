...
import { auditProtectInfo, delProtectInfo, getProtectInfo } from '@/api/collection-all/manage-work/protect-info'
import CommHandleBtnTable from '@/components/comm-cpns/comm-handle-btn-table/comm-handle-btn-table.vue'
import { auditPassOnExperience } from '@/api/collection-all/manage-work/pass-on-experience'
...
async function handleGetList() {
  try {
    ...
    const res = await getProtectInfo(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '保护信息-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '保护信息-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleDelete() {
  ...
  const msg = selectionArr.value?.map(row => {
    return `${row?.protectionTechnologyCategories}`
  })
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 保护技术类别: ${msg} 数据项？`)
    .then(() => delProtectInfo({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ...
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 保护技术类别: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditProtectInfo(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
