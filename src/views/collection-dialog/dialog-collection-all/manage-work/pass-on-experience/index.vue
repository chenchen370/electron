...
import { auditPassOnExperience, delPassOnExperience, getPassOnExperience } from '@/api/collection-all/manage-work/pass-on-experience'
import { auditAppendGoods } from '@/api/collection-all/index-group/append-goods'
import { auditMoveRecord } from '@/api/collection-all/manage-work/move-record'
...
async function handleGetList() {
  try {
    ...
    const res = await getPassOnExperience(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '流传经历-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '流传经历-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleDelete() {
  ...
  const msg = selectionArr.value?.map(row => {
    return `${row?.collectionOrganization}`
  })
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 收藏机构: ${msg} 数据项？`)
    .then(() => delPassOnExperience({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ...
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 收藏机构: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditPassOnExperience(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
