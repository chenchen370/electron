...
import { auditAccDamOccRecord, delAccDamOccRecord, getAccDamOccRecord } from '@/api/collection-all/manage-work/accident-damage-occurred-record'
import CommHandleBtnTable from '@/components/comm-cpns/comm-handle-btn-table/comm-handle-btn-table.vue'
import { auditRepairRecord } from '@/api/collection-all/manage-work/repair-record'
...
async function handleGetList() {
  try {
    ...
    const res = await getAccDamOccRecord(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '事故损坏发生记录-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '事故损坏发生记录-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 事故地点: ${msg} 数据项？`)
    .then(() => delAccDamOccRecord({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ?.map(row => {
      console.log('row?.accidentLocation', row)
      return `${row?.accidentLocation}`
    })
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 事故地点: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditAccDamOccRecord(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
