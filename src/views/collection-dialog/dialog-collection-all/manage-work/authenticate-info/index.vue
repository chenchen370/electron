...
import { auditAuthenticate, delAuthenticate, getAuthenticate } from '@/api/collection-all/manage-work/authenticate-info'
import { auditFileInfo } from '@/api/collection-all/index-group/file-info'
...
async function handleGetList() {
  try {
    ...
    const res = await getAuthenticate(params)
...
function handleAdd() {
  console.log('dd')
  rowObj.value = {}
  dialogInfo.title = '鉴定信息-新增'
  dialogInfo.isShowDialogAddOrUpdate = true
}

// header 修改
function handleUpdate() {
  dialogInfo.title = '鉴定信息-修改'
  dialogInfo.isShowDialogAddOrUpdate = true
}
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    .confirm(`是否删除 鉴定机构: ${msg} 数据项？`)
    .then(() => delAuthenticate({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
...
async function audit() {
  ...
  const msg = selectionArr.value
    ...
    .join('、')
  props.proxy.$modal
    .confirm(`是否提交审核 鉴定机构: ${msg} 数据项？`)
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditAuthenticate(valueElement)
      }
    })
    .then(async () => {
      await handleGetList()
