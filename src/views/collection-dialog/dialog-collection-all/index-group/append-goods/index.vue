...
import { auditAppendGoods, delAppendGoods, getAppendGoods } from '@/api/collection-all/index-group/append-goods'
import { auditFileInfo, delFileInfo } from '@/api/collection-all/index-group/file-info'
import { auditAccDamOccRecord } from '@/api/collection-all/manage-work/accident-damage-occurred-record'
...
async function handleGetList() {
  try {
    ...
    const res = await getAppendGoods(params)
...
function handleRowDelete(idList, msg) {
  props.proxy.$modal
    ...
    .then(() => delAppendGoods({ id: idList.join(','), relicsId: props.relicsId }))
    .then(() => {
      handleGetList()
  ...
  props.proxy.$modal
    ...
    .then(async () => {
      for (let valueElement of selectionArr.value) {
        await auditAppendGoods(valueElement)
      }
    })
    .then(() => {
      handleGetList()
