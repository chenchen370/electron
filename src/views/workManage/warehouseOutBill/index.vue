  ...
  <div class="app-container">
    <el-card shadow="never" class="mb-10px">
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseOutBill:add']" title="新增" img-name="add"
          ...
          <!--            icon="Plus"-->
          <!--            @click="handleAdd"-->
          <!--            v-hasPermi="['workManage:warehouseOutBill:add']"-->
          <!--            >新增</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseOutBill:edit']" title="修改" icon-name="edit" :disabled="single"
          ...
          <!--            :disabled="single"-->
          <!--            @click="handleUpdate"-->
          <!--            v-hasPermi="['workManage:warehouseOutBill:edit']"-->
          <!--            >修改</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseOutBill:remove']" title="删除" icon-name="remove"
          ...
          <!--            :disabled="multiple"-->
          <!--            @click="handleDelete"-->
          <!--            v-hasPermi="['workManage:warehouseOutBill:remove']"-->
          <!--            >删除</el-button-->
          <!--          >-->
        </el-col>
      ...
      <el-table style="width: 100%" v-loading="loading" :data="warehouseOutBillList"
        ...
        <el-table-column label="操作" fixed="right" align="center" width="200" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="EditPen" :size="22" title="编辑" @click="handleUpdate(scope.row)"
              v-hasPermi="['workManage:warehouseOutBill:edit']"></el-button>
            <el-button link type="primary" icon="Delete" :size="22" title="删除" @click="handleDelete(scope.row)"
              v-hasPermi="['workManage:warehouseOutBill:remove']"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="800px" append-to-body @close="cancel" align-center
      :show-close="false">
      <el-form ref="warehouseOutBillRef" :model="form" :rules="rules" label-width="auto" size="large">
      </el-form>
      <template #footer>
...
<script setup name="WarehouseOutBill">
// import {
//   listWarehouseOutBill,
//   getWarehouseOutBill,
//   delWarehouseOutBill,
//   addWarehouseOutBill,
//   updateWarehouseOutBill
// } from '@/api/workManage/warehouseOutBill';

import CommHandleBtn from '@/components/comm-cpns/comm-handle-btn/comm-handle-btn.vue';

const { proxy } = getCurrentInstance();

const warehouseOutBillList = ref([]);
...
function getList() {
  loading.value = false;
  // listWarehouseOutBill(proxy.addDateRange(queryParams.value, dateTimeRange.value)).then(response => {
  //   warehouseOutBillList.value = response.data.records;
...
function reset() {
  ...
  proxy.resetForm('warehouseOutBillRef');
}
...
function handleUpdate(row) {
  ...
  getWarehouseOutBill(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs['warehouseOutBillRef'].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateWarehouseOutBill(form.value).then(response => {
          proxy.$modal.msgSuccess('修改成功');
          open.value = false;
          getList();
        });
      } else {
        addWarehouseOutBill(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal
    ...
    .then(function () {
      return delWarehouseOutBill(_ids);
    })
    .then(() => {
      getList();
