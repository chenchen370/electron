  ...
  <div class="app-container">
    <el-card shadow="never" class="mb-10px">
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseLogoutBill:add']" title="新增" img-name="add"
          ...
          <!--            icon="Plus"-->
          <!--            @click="handleAdd"-->
          <!--            v-hasPermi="['workManage:warehouseLogoutBill:add']"-->
          <!--            >新增</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseLogoutBill:edit']" title="修改" icon-name="edit" :disabled="single"
          ...
          <!--            :disabled="single"-->
          <!--            @click="handleUpdate"-->
          <!--            v-hasPermi="['workManage:warehouseLogoutBill:edit']"-->
          <!--            >修改</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseLogoutBill:remove']" title="删除" icon-name="remove"
          ...
          <!--            :disabled="multiple"-->
          <!--            @click="handleDelete"-->
          <!--            v-hasPermi="['workManage:warehouseLogoutBill:remove']"-->
          <!--            >删除</el-button-->
          <!--          >-->
        </el-col>
      ...
      <el-table style="width: 100%" v-loading="loading" :data="warehouseLogoutBillList"
    ...
    <!-- 添加或修改对话框 -->
    <el-dialog :title="title" v-model="open" width="800px" append-to-body @close="cancel" align-center :show-close="false">
      <el-form ref="warehouseLogoutBillRef" :model="form" :rules="rules" label-width="auto" size="large">
      </el-form>
      <template #footer>
...
<script setup name="WarehouseLogoutBill">
// import {
//   listWarehouseLogoutBill,
//   getWarehouseLogoutBill,
//   delWarehouseLogoutBill,
//   addWarehouseLogoutBill,
//   updateWarehouseLogoutBill
// } from '@/api/workManage/warehouseLogoutBill';

import CommHandleBtn from '@/components/comm-cpns/comm-handle-btn/comm-handle-btn.vue';

const { proxy } = getCurrentInstance();

const warehouseLogoutBillList = ref([]);
...
function getList() {
  loading.value = false;
  // listWarehouseLogoutBill(proxy.addDateRange(queryParams.value, dateTimeRange.value)).then(response => {
  //   warehouseLogoutBillList.value = response.data.records;
...
function reset() {
  ...
  proxy.resetForm('warehouseLogoutBillRef');
}
...
function handleUpdate(row) {
  ...
  getWarehouseLogoutBill(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs['warehouseLogoutBillRef'].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateWarehouseLogoutBill(form.value).then(response => {
          proxy.$modal.msgSuccess('修改成功');
          open.value = false;
          getList();
        });
      } else {
        addWarehouseLogoutBill(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal
    ...
    .then(function () {
      return delWarehouseLogoutBill(_ids);
    })
    .then(() => {
      getList();
