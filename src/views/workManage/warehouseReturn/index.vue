  ...
  <div class="app-container">
    <el-card shadow="never" class="mb-10px">
      ...
      <el-row :gutter="10" class="mb8">
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseReturn:add']" title="新增" img-name="add"
          ...
          <!--            icon="Plus"-->
          <!--            @click="handleAdd"-->
          <!--            v-hasPermi="['workManage:warehouseReturn:add']"-->
          <!--            >新增</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseReturn:edit']" title="修改" img-name="edit" :disabled="single"
          ...
          <!--            :disabled="single"-->
          <!--            @click="handleUpdate"-->
          <!--            v-hasPermi="['workManage:warehouseReturn:edit']"-->
          <!--            >修改</el-button-->
          <!--          >-->
        </el-col>
        <el-col :span="1.5">
          <comm-handle-btn :per="['workManage:warehouseReturn:remove']" title="删除" img-name="remove"
          ...
          <!--            :disabled="multiple"-->
          <!--            @click="handleDelete"-->
          <!--            v-hasPermi="['workManage:warehouseReturn:remove']"-->
          <!--            >删除</el-button-->
          <!--          >-->
        </el-col>
      ...
      <el-table style="width: 100%" v-loading="loading" :data="warehouseReturnList"
        ...
        <el-table-column label="操作" fixed="right" align="center" width="200" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button link type="primary" icon="EditPen" :size="22" title="编辑" @click="handleUpdate(scope.row)"
              v-hasPermi="['workManage:warehouseReturn:edit']"></el-button>
            <el-button link type="primary" icon="Delete" :size="22" title="删除" @click="handleDelete(scope.row)"
              v-hasPermi="['workManage:warehouseReturn:remove']"></el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="800px" append-to-body @close="cancel" align-center
      :show-close="false">
      <el-form ref="warehouseReturnRef" :model="form" :rules="rules" label-width="auto" size="large">
...
<script setup name="WarehouseReturn">
// import {
//   listWarehouseReturn,
//   getWarehouseReturn,
//   delWarehouseReturn,
...
function getList() {
  loading.value = false;
  // listWarehouseReturn(proxy.addDateRange(queryParams.value, dateTimeRange.value)).then(response => {
  //   warehouseReturnList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm('warehouseReturnRef');
}
...
function handleUpdate(row) {
  ...
  getWarehouseReturn(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs['warehouseReturnRef'].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateWarehouseReturn(form.value).then(response => {
          proxy.$modal.msgSuccess('修改成功');
          open.value = false;
          getList();
        });
      } else {
        addWarehouseReturn(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal
    ...
    .then(function () {
      return delWarehouseReturn(_ids);
    })
    .then(() => {
      getList();
