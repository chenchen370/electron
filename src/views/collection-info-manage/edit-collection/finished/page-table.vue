<template>
  <div>
    <el-table style="width: 100%" v-loading="queryOpt.loading" :data="queryOpt.thingList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="审核人" align="center" prop="approvalBy" />
...
import { options_approval_status, options_approval_type } from '../../dict'
...
// 查看 媒体 信息
function handleOpenMediaKindsList(item) {
  emits('handleOpenMediaKindsList', item)
}

// 查看 藏品 all信息
function handleOpenCollectionAll(item) {
  emits('handleOpenCollectionAll', item)
}
