  ...
  <el-dialog :title="`${proFormProps.title}`" v-model="proFormVisible" width="1028px" @close="close" align-center
    ...
    <div class="mn-container">
      <el-form ref="formRef" :model="form" :disabled="proFormProps.isView" :rules="rules" label-position="left"
        ...
        <el-row :gutter="50">
          <el-col :span="16">
            <el-form-item label="展柜编号" prop="cabinetId" style="margin-bottom: 0;">
              <div class="td-font_tip">
                编号为数字或数字和字母的综合，不能重复。建议：如可命名为001
              </div>
            </el-form-item>
            <el-input v-model="form.cabinetId" placeholder="" disabled>
              <!-- <template #prepend>ZG</template> -->
            </el-input>
          </el-col>
          <el-col :span="8">
            <el-form-item label="展柜名称" prop="cabinetName" class="td-flex column">
          ...
          <el-col :span="8">
            <el-form-item label="所属展厅" prop="houseId" class="td-flex column">
          ...
          <el-col :span="8">
            <el-form-item label="具体位置" prop="cabinetAddrDetail" class="td-flex column">
              <el-input v-model="form.cabinetAddrDetail" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="展柜类型" prop="cabinetType" class="td-flex column">
              <el-select v-model="form.cabinetType" placeholder="请选择">
                <el-option v-for="item in display_cabinet_type" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="16">
            <el-form-item label="展柜大小（m）" prop="cabinetVolume" class="td-flex column">
              <div class="td-flex">
          ...
          <el-col :span="8">
            <el-form-item label="展柜型号" prop="modelNum" class="td-flex column">
              <el-input v-model="form.modelNum" placeholder="" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="展柜功能类型" prop="funcType" class="td-flex column">
          ...
          <el-col :span="8">
            <el-form-item prop="funcContent" label="功能描述" style="margin-bottom: 0;">
              <div class="td-font_tip">
                若含控制设备，请描述具体控制内容
              </div>
            </el-form-item>
          ...
          <el-col :span="8">
            <el-form-item label="展柜厂商" prop="facturer" class="td-flex column">
          ...
          <el-col :span="8">
            <el-form-item label="文物类别" prop="artifactType" class="td-flex column">
              <el-select v-model="form.artifactType" placeholder="请选择">
                <el-option v-for="item in artifact_type" :key="item.value" :label="item.label"
                  :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item prop="cpTextureDic" label="藏品质地" class="td-flex column">
              <cusSelectTree :readOnly="proFormProps.title === '详情'" :modelValue="form.cpTextureDic"
                @update="getcpTextureDic">
              </cusSelectTree>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="展览商品">
              <div class="i-link td-pointer">下载当前展柜藏品信息</div>
            </el-form-item>
            <el-table class="td-mt20" :data="additionalList" border>
              <el-table-column label="展柜编号" prop="houseName" show-overflow-tooltip />
              <el-table-column label="藏品归属" prop="houseName" show-overflow-tooltip />
              <el-table-column label="藏品编号名称" prop="pricingUnit" show-overflow-tooltip />
              <el-table-column label="藏品编号" prop="houseName" show-overflow-tooltip />
              <el-table-column label="藏品名称" prop="unitPrice" show-overflow-tooltip />
            </el-table>
          </el-col>
        </el-row>
...
<script setup>
import {
  createIdDisplayCabinet,
} from '@/api/artifact/displayCabinet';
import { reactive } from 'vue';
import { ElMessage } from 'element-plus';
const { proxy } = getCurrentInstance();
const { display_cabinet_type, func_type, moniting_status, artifact_type } =
  proxy.useDict(
...
const rules = reactive({
  ...
  cabinetType: [{ required: true, trigger: 'blur', message: '请输入' }],
  houseId: [{ required: true, trigger: 'blur', message: '请输入' }],
  cabinetVolume: [{ required: false, trigger: 'blur', message: '请输入' }],
  facturerTel: [
    {
...
const getcpTextureDic = v => {
  form.value.cpTextureDic = v.label;
  form.value.cpTexture = v.value;
};
const reset = () => {
  form.value = {
    cabinetId: '',
...
const createId = async () => {
  const { data } = await createIdDisplayCabinet();
