...
import { getAreaHisMonitorData } from '@/api/warning/excel';
...
const handleSubmit = async () => {
  ...
  const params = {
    warningLocationId: form.value.areaId,
    startDate: form.value.startDate,
    endDate: form.value.endDate
  };
  const { rows } = await getMonitorDataHis(params);
...
const doDownload = () => {
  ...
  const params = 'warningLocationId=' + form.value.areaId + '&startDate=' + form.value.startDate + '&endDate=' + form.value.endDate
  getAreaHisMonitorData(params);
}
const acceptParams = params => {
  proFormProps.value = params;
  form.value['areaId'] = params.row?.cabinetId || params.row?.houseId
