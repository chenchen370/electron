...
export declare namespace Table {
  ...
  export interface DataCallBack {
    (
      ...
      | { data: any[] }
      | { list: any[]; total: number; pageSize?: number; pageNum?: number }
  }
}
/**
...
export const useTable = (
  ...
  const getTableList = async () => {
    ...
    try {
      ...
      let data = res.rows ?? res.data?.records
      ...
      if (isPageable) {
        const realRes = res?.data?.total ? res.data : res
        const { pageNum, pageSize, total } = realRes
        console.log('realRes', realRes)
        let pageNumVal = pageNum ? pageNum : state.pageable.pageNum
        let pageSizeVal = pageSize ? pageSize : state.pageable.pageSize
        let totalVal = !isNaN(total) ? total : state.pageable.total
        // console.log(pageNumVal, pageSizeVal, total);
        updatePageable({
          pageNum: pageNumVal,
