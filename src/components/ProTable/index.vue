<template>
  <div class="el-protable">

    <div :class="['el-protable-main', noHasTab ? 'td-p18' : '']">
      <!-- 表格内容 card -->
      <!-- <el-card shadow="never"> -->
      <div class="el-protable-search" :style="isShowSearch ? { marginBottom: '10px' } : ''">
        <SearchForm v-show="isShowSearch" :search="tableEmits.search" :reset="tableEmits.reset" :columns="searchColumns"
          :search-param="searchParam" :search-col="searchCol" />
      </div>

      <!-- 表格头部 操作按钮 -->
      <div class="el-protable-main-header">
        <div class="el-protable-main-header__left">
          <span class="el-protable-main-header__left-btn">
            <slot name="action" :selected-list-ids="selectedListIds" :selected-list="selectedList"
              :is-selected="isSelected" />
          </span>
          <span class="el-protable-main-header__left-title">
            <slot name="title">
              {{ props.title }}
            </slot>
          </span>
          <span class="el-protable-main-header__action">
            <slot name="tools" :selected-list-ids="selectedListIds" :selected-list="selectedList"
              :is-selected="isSelected" />
          </span>
        </div>
        <div v-if="!!toolButton" class="el-protable-main__action">
          <el-divider v-if="$slots['tools']" direction="vertical"></el-divider>
          <slot name="toolButton">
            ...
            <el-popover v-if="showToolButtonItem('setting') && columns.length" trigger="click" width="fit-content">
              ...
              <el-tree ref="colSettingRef" :data="tableColumns" node-key="prop" show-checkbox draggable
                :allow-drop="allowDrop" :props="{ children: '_children' }" @node-drop="dropSuccess"
                :filter-node-method="filterTreeNode" @check-change="treeCheckChange">
            ...
            <el-button v-if="showToolButtonItem('search') && searchColumns.length" :icon="Search" circle
              @click="isShowSearch = !isShowSearch" />
          </slot>
        </div>
      </div>
      <!-- 表格主体 -->
      <el-table ref="tableRef" v-loading="loading" v-bind="$attrs" :data="cacheData" :border="border" :row-key="rowKey"
        ...
        <template v-for="item in tableColumns" :key="item">
          ...
          " v-bind="item" :align="item.align ?? 'center'" :reserve-selection="item.type == 'selection'"
      ...
      <slot name="pagination">
        <!-- 分页组件 -->
        <el-pagination v-if="!!pagination" v-bind="paginationProps" :current-page="pageable.pageNum"
          :page-size="pageable.pageSize" :total="pageable.total" @size-change="handleSizeChange"
...
import { useTable, useSelection } from './hooks/index';
import {
  Refresh,
  Operation,
  Search,
...
const dropSuccess = (draggingNode: any) => {
  if (draggingNode.checked)
    colSettingRef.value.setChecked(draggingNode.key, true, true);
};
// 过滤隐藏的column
const filterTreeNode = (_value: any, data: any) => {
  return data.isShow && !data.type;
};

// tree check事件
const treeCheckChange = (data: any, checked: boolean, leaf: boolean) => {
...
// 接收 columns 并设置为响应式
const tableColumns = reactive(props.columns as any);
...
const setEnumMap = async (col: any) => {
  ...
  if (
    ...
    (typeof col.enum === 'function' ||
      enumMap.value.get(col.prop!) === col.enum)
  )
    return;
...
const flatColumnsFunc = (columns: any, flatArr: any = []) => {
  ...
  return flatArr.filter((item: any) => !item._children?.length);
};
...
const searchColumns = computed(() => {
  return (
    flatColumns.value
      ?.filter((item: any) => item.search?.el || item.search?.render)
      .sort((a: any, b: any) => a.search!.order! - b.search!.order!) ?? []
  );
});
// 是否显示搜索模块
...
searchColumns.value.forEach((column: any, index: any) => {
...
const dragSort = () => {
  ...
  Sortable.create(tbody, {
    ...
    animation: 200,
    onUpdate({ newIndex, oldIndex }: any) {
